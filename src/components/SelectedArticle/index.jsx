...
const SelectedArticle = ({ currentArticle, updateArticle, currentCategory, updateCategory }) => {
  return (
    <>
    <div className={styles.currentArticle}>
      ...
      <p className={styles.articleDeck}>{currentArticle.article.deck}</p><br />
      <p className={styles.articleDeck}>{currentArticle.article.story}</p>
    </div>
    <h3 className={styles.backBtn} onClick={() => (updateArticle(null), updateCategory(currentCategory))}>Back</h3>
    </>
  );
};
